 ...
 */
 ...
 private:
  ...
  std::vector<std::pair<std::thread, bool>> wts;
  std::vector<std::pair<std::function<void(void)>, std::unique_ptr<semaphore>>> functions;
  semaphore threads;
  void dispatcher();
  std::mutex qLock;
  std::mutex wtsLock;
  std::mutex funcsLock;
  std::condition_variable_any qCond, workerCond;
  std::queue<std::function<void(void)>> thunkQ;
