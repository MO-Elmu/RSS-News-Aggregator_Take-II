 ...
 */
  ...
  /* Helper methods to make processAllFeeds() fully multithreaded */
  void getRssFeeds(std::string url);

  void getFeedArticles(Article artcl);
...
/* Data structures and mutexes for threading operations */

std::set<Article> urlTracker;
std::mutex IndexLock;
std::mutex SetLock;
ThreadPool feedPool;
ThreadPool artclPool;
};
